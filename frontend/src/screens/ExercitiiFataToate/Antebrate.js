...
const defaultExercitiiData = {
  "Seated Barbell Wrist Curl": {
    descriere: "Exercițiu izolat pentru antebrațe. Așezat pe o bancă, cu antebrațele pe picioare și încheieturile peste genunchi, ții o bară cu palmele în sus și miști doar încheieturile pentru a ridica și coborî bara. Lucrează intens mușchii flexori ai încheieturii.",
    video: "https://youtu.be/cOBaYeX3bYo",
    animatie: require('../../../assets/gifs/wrist-curl.png'),
    muschiAntrenati: "Flexori ai încheieturii",
    grupaMusculara: "Antebrațe",
    equipment: "Bară dreaptă și bancă",
    isDefault: true
  }
};
...
const Antebrate = () => {
  ...
  const [grupaMusculara, setGrupaMusculara] = useState('Antebrațe');
  ...
  const incarcaExercitii = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/exercitii?grupaMusculara=Antebrațe`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      const exercitiiAntebrate = data.filter(exercitiu => exercitiu.grupaMusculara === 'Antebrațe');
      console.log(`Am filtrat ${exercitiiAntebrate.length} exerciții pentru antebrațe`);
      
      const exercitiiFormatate = { ...defaultExercitiiData };
      
      exercitiiAntebrate.forEach(exercitiu => {
        if (!exercitiiFormatate[exercitiu.nume]) {
          ...
          if (exercitiu.animatie) {
            // Verificăm dacă animația este deja un URL complet
      ...
      console.log('Exercițiile pentru antebrațe au fost încărcate cu succes');
    } catch (error) {
  ...
  const adaugaExercitiu = async () => {
    ...
    try {
      ...
      console.log('Răspuns adăugare exercițiu:', responseData);
      ...
      if (imagineSursa) {
        ...
        console.log('URL pentru încărcare:', `${API_URL}/exercitii/${encodeURIComponent(numeExercitiu)}/imagine`);
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.titlu}>Antebrațe</Text>
