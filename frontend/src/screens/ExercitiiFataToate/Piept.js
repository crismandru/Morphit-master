...
const defaultExercitiiData = {
  "Bench Press": {
    descriere: "Exercițiu compus pentru piept. Întins pe o bancă, împingi bara de la nivelul pieptului până la extensie completă a brațelor. Lucrează pectoralii majori, deltoizii anteriori și tricepsul.",
    video: "https://youtu.be/tuwHzzPdaGc",
    animatie: require('../../../assets/gifs/bench-press.png'),
    muschiAntrenati: "Pectorali majori",
    grupaMusculara: "Piept",
    equipment: "Bancă și bară",
    isDefault: true
  },
  "Dumbbell Flys": {
    descriere: "Exercițiu pentru partea superioară a pieptului. Executat pe o bancă înclinată, împingi ganterele de la nivelul pieptului până la extensie completă a brațelor. Lucrează mai mult partea superioară a pectoralilor.",
    video: "https://youtu.be/-lcbvOddoi8",
    animatie: require('../../../assets/gifs/dumbbell-flys.png'),
    muschiAntrenati: "Pectorali superiori",
    grupaMusculara: "Piept",
    equipment: "Bancă și gantere",
    isDefault: true
  }
};
...
const Piept = () => {
  ...
  const [grupaMusculara, setGrupaMusculara] = useState('Piept');
  ...
  const incarcaExercitii = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/exercitii?grupaMusculara=Piept`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      const exercitiiPiept = data.filter(exercitiu => exercitiu.grupaMusculara === 'Piept');
      console.log(`Am filtrat ${exercitiiPiept.length} exerciții pentru Piept`);
      
      const exercitiiFormatate = { ...defaultExercitiiData };
      
      exercitiiPiept.forEach(exercitiu => {
      ...
      console.log('Exercițiile pentru Piept au fost încărcate cu succes');
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.titlu}>Piept</Text>
