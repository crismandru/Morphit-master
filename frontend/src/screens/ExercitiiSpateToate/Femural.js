...
const defaultExercitiiData = {
  "RDLs": {
    descriere: "Exercițiu compus pentru mușchii ischiojambieri și fesieri. Stai drept cu picioarele la lățimea umerilor, ții bara cu ambele mâini în fața coapselor. Înclină corpul înainte de la șolduri, menținând spatele drept și genunchii ușor îndoiți, până când simți o întindere în ischiojambieri. Ridică controlat înapoi în poziția inițială, activând mușchii fesieri.",
    video: "https://youtu.be/-m45n1_x32E",
    animatie: require('../../../assets/gifs/rdl.png'),
    muschiAntrenati: "Ischiojambieri, Fesieri, Spate inferior",
    grupaMusculara: "Femural",
    equipment: "Bară dreaptă",
    isDefault: true
  },
  "Seated Leg Curl": {
    descriere: "Exercițiu izolat pentru mușchii ischiojambieri. Așezat la aparatul de leg curl, cu spatele drept și picioarele așezate pe suportul mobil. Tragi suportul în jos prin îndoirea genunchilor, menținând coapsele pe banca aparatului. Cobori controlat înapoi în poziția inițială, menținând tensiunea constantă în mușchii ischiojambieri.",
    video: "https://youtu.be/3BWiLFc8Dbg",
    animatie: require('../../../assets/gifs/seated-leg-curl.png'),
    muschiAntrenati: "Ischiojambieri",
    grupaMusculara: "Femural",
    equipment: "Aparat Seated Leg Curl",
    isDefault: true
  }
};
...
const Femural = () => {
  ...
  const [grupaMusculara, setGrupaMusculara] = useState('Femural');
  ...
  const incarcaExercitii = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/exercitii?grupaMusculara=Femural`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      const exercitiiFemural = data.filter(exercitiu => exercitiu.grupaMusculara === 'Femural');
      console.log(`Am filtrat ${exercitiiFemural.length} exerciții pentru Femural`);
      
      const exercitiiFormatate = { ...defaultExercitiiData };
      
      exercitiiFemural.forEach(exercitiu => {
      ...
      console.log('Exercițiile pentru Femural au fost încărcate cu succes');
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      <TouchableOpacity onPress={() => navigation.navigate('ExercitiiSpate')} style={styles.butonInapoi}>
        <Icon name="arrow-back" size={30} color="#fff" />
      </TouchableOpacity>

      <Text style={styles.titlu}>Femural</Text>
