...
const defaultExercitiiData = {
  "Lat Pull Down": {
    descriere: "Exercițiu compus pentru mușchii latissimus dorsi. Așezat la aparatul de lat pull down, cu picioarele fixate sub suport, ții bara cu ambele mâini la lățimea umerilor. Tragi bara în jos până la nivelul pieptului, menținând spatele drept și coatele îndreptate în jos, apoi ridici controlat bara înapoi în poziția inițială.",
    video: "https://youtu.be/iKrKgWR9wbY",
    animatie: require('../../../assets/gifs/lat-pull-down.png'),
    muschiAntrenati: "Latissimus dorsi, Trapez, Biceps",
    grupaMusculara: "Spate",
    equipment: "Aparat Lat Pull Down",
    isDefault: true
  },
  "Bent Over Row": {
    descriere: "Exercițiu compus pentru mușchii spatelui. Înclinat înainte cu spatele drept, cu genunchii ușor îndoiți, ții bara cu ambele mâini la lățimea umerilor. Tragi bara spre abdomen, menținând coatele apropiate de corp, apoi cobori controlat bara înapoi în poziția inițială. Menține spatele drept și abdomenul încordat pe tot parcursul exercițiului.",
    video: "https://youtu.be/paCfxhgW6bI",
    animatie: require('../../../assets/gifs/bent-over-row.png'),
    muschiAntrenati: "Latissimus dorsi, Trapez, Rhomboids, Biceps",
    grupaMusculara: "Spate",
...
const Spate = () => {
  ...
  const [grupaMusculara, setGrupaMusculara] = useState('Spate');
  ...
  const incarcaExercitii = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/exercitii?grupaMusculara=Spate`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      const exercitiiSpate = data.filter(exercitiu => exercitiu.grupaMusculara === 'Spate');
      console.log(`Am filtrat ${exercitiiSpate.length} exerciții pentru Spate`);
      
      const exercitiiFormatate = { ...defaultExercitiiData };
      
      exercitiiSpate.forEach(exercitiu => {
      ...
      console.log('Exercițiile pentru Spate au fost încărcate cu succes');
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.titlu}>Spate</Text>
