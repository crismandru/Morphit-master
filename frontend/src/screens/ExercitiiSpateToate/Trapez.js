...
const defaultExercitiiData = {
  "Dumbbell Shrug": {
    descriere: "Exercițiu izolat pentru trapez. Stai în picioare cu ganterele în mâini de-a lungul corpului, cu palmele spre corp. Ridici umerii cât mai sus posibil, menținând brațele drepte, apoi cobori controlat în poziția inițială. Concentrează-te pe contracția trapezului superior.",
    video: "https://youtu.be/dj2Gm628kas",
    animatie: require('../../../assets/gifs/dumbbell-shrug.png'),
    muschiAntrenati: "Trapez superior",
    grupaMusculara: "Trapez",
    equipment: "Gantere",
    isDefault: true
  },
  "Barbell Shrug": {
    descriere: "Exercițiu izolat pentru trapez. Stai în picioare cu bara în mâini în fața coapselor, cu palmele spre corp. Ridici umerii cât mai sus posibil, menținând brațele drepte, apoi cobori controlat în poziția inițială. Poate fi executat și în spatele corpului pentru o variație diferită.",
    video: "https://youtu.be/6hNudn7Peco",
    animatie: require('../../../assets/gifs/barbell-shrug.png'),
    muschiAntrenati: "Trapez superior",
    grupaMusculara: "Trapez",
...
const Trapez = () => {
  ...
  const [grupaMusculara, setGrupaMusculara] = useState('Trapez');
  ...
  const incarcaExercitii = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/exercitii?grupaMusculara=Trapez`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      const exercitiiTrapez = data.filter(exercitiu => exercitiu.grupaMusculara === 'Trapez');
      console.log(`Am filtrat ${exercitiiTrapez.length} exerciții pentru Trapez`);
      
      const exercitiiFormatate = { ...defaultExercitiiData };
      
      exercitiiTrapez.forEach(exercitiu => {
      ...
      console.log('Exercițiile pentru Trapez au fost încărcate cu succes');
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.titlu}>Trapez</Text>
