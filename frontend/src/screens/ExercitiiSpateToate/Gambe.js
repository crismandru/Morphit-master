...
const defaultExercitiiData = {
  "Seated Calf Raise": {
    descriere: "Exercițiu izolat pentru mușchii găști. Așezat la aparatul de calf raise, cu vârfurile picioarelor pe suportul mobil și genunchii sub pernele aparatului. Ridici călcâiele cât mai sus posibil prin contracția mușchilor găști, menținând tensiunea în vârf. Cobori controlat înapoi în poziția inițială, până când călcâiele sunt sub nivelul suportului.",
    video: "https://youtu.be/Yh5TXz99xwY",
    animatie: require('../../../assets/gifs/seated-calf-raise.png'),
    muschiAntrenati: "Găști",
    grupaMusculara: "Gambe",
    equipment: "Aparat Seated Calf Raise",
    isDefault: true
  },
  "Standing Machine Calf Raise": {
    descriere: "Exercițiu izolat pentru mușchii găști. Stai drept la aparatul de calf raise, cu umerii sub pernele aparatului și vârfurile picioarelor pe suportul mobil. Ridici călcâiele cât mai sus posibil prin contracția mușchilor găști, menținând tensiunea în vârf. Cobori controlat înapoi în poziția inițială, până când călcâiele sunt sub nivelul suportului.",
    video: "https://youtu.be/RBslMmWqzzE",
    animatie: require('../../../assets/gifs/standing-machine-calf-raise.png'),
    muschiAntrenati: "Găști",
    grupaMusculara: "Gambe",
    equipment: "Aparat Standing Calf Raise",
    isDefault: true
  }
};
...
const Gambe = () => {
  ...
  const [grupaMusculara, setGrupaMusculara] = useState('Gambe');
  ...
  const incarcaExercitii = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/exercitii?grupaMusculara=Gambe`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      const exercitiiGambe = data.filter(exercitiu => exercitiu.grupaMusculara === 'Gambe');
      console.log(`Am filtrat ${exercitiiGambe.length} exerciții pentru Gambe`);
      
      const exercitiiFormatate = { ...defaultExercitiiData };
      
      exercitiiGambe.forEach(exercitiu => {
      ...
      console.log('Exercițiile pentru Gambe au fost încărcate cu succes');
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.titlu}>Gambe</Text>
