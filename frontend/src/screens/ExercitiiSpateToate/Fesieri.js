...
const defaultExercitiiData = {
  "Barbell Hip Thrusts": {
    descriere: "Exercițiu izolat pentru mușchii fesieri. Așezat pe o bancă, cu spatele drept, cu bara pe șolduri. Genunchii sunt îndoiți la 90 de grade, iar picioarele sunt așezate pe podea. Ridici șoldurile în sus până când corpul formează o linie dreaptă de la genunchi până la umeri, apoi cobori controlat înapoi în poziția inițială. Menține tensiunea constantă în mușchii fesieri.",
    video: "https://youtu.be/lAnqN0J_p5A",
    animatie: require('../../../assets/gifs/hip-thrust.png'),
    muschiAntrenati: "Fesieri, Ischiojambieri",
    grupaMusculara: "Fesieri",
...
const Fesieri = () => {
  ...
  const [grupaMusculara, setGrupaMusculara] = useState('Fesieri');
  ...
  const incarcaExercitii = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/exercitii?grupaMusculara=Fesieri`, {
        method: 'GET',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
      ...
      const exercitiiFesieri = data.filter(exercitiu => exercitiu.grupaMusculara === 'Fesieri');
      console.log(`Am filtrat ${exercitiiFesieri.length} exerciții pentru Fesieri`);
      
      const exercitiiFormatate = { ...defaultExercitiiData };
      
      exercitiiFesieri.forEach(exercitiu => {
      ...
      console.log('Exercițiile pentru Fesieri au fost încărcate cu succes');
    } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.titlu}>Fesieri</Text>
